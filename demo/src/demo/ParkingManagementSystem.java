...
public class ParkingManagementSystem extends JFrame {
    ...
    private JPanel buildLoginPanel() {
        JPanel root = new JPanel(new GridBagLayout()) {
            @Override protected void paintComponent(Graphics g) {
                ...
                g2.setColor(C_ACCENT);  g2.fillOval(-100, -100, 400, 400);
                g2.setColor(C_ACCENT2); g2.fillOval(getWidth()-300, getHeight()-300, 500, 500);
        ...
        sub.setFont(F_BODY); sub.setForeground(C_TEXT_DIM);
        ...
        errLabel.setForeground(C_DANGER); errLabel.setFont(F_BODY);
        ...
        card.add(fieldLabel("Username")); card.add(Box.createVerticalStrut(6)); card.add(userField);
        card.add(Box.createVerticalStrut(14));
        card.add(fieldLabel("Password")); card.add(Box.createVerticalStrut(6)); card.add(passField);
        card.add(Box.createVerticalStrut(22));
        card.add(loginBtn); card.add(Box.createVerticalStrut(10)); card.add(errLabel);

        ActionListener doLogin = e -> {
            String u = userField.getText().trim();
            String p = new String(passField.getPassword()).trim();
            if (u.isEmpty() || p.isEmpty()) { errLabel.setText("Enter both fields"); return; }
            if (authAdmin(u, p)) { currentUser = u; errLabel.setText(" "); showCard("Dashboard"); }
    ...
    private JPanel buildDashboard() {
        ...
        lbl_total    = new JLabel("0"); lbl_avail = new JLabel("0");
        lbl_occupied = new JLabel("0"); lbl_revenue = new JLabel("0");
        ...
        for (String[] item : nav) {
            JButton b = navButton(item[0]); String c = item[1];
            b.addActionListener(e -> showCard(c)); navRow.add(b);
        }
        ...
        tblTitle.setFont(F_LABEL); tblTitle.setForeground(C_TEXT);
        tableCard.add(tblTitle, BorderLayout.NORTH);
        JScrollPane sp = new JScrollPane(tbl);
        sp.setBorder(null); sp.getViewport().setBackground(C_CARD);
    ...
    private void refreshDashboard() {
        ...
        try {
            ...
            try (Statement s = conn.createStatement();
                ...
                if (r.next()) lbl_revenue.setText("Rs" + fmt(r.getDouble("rev")));
            }
            mdl_parked.setRowCount(0);
            String sql =
                "SELECT v.id, v.vehicle_number, v.vehicle_type, v.owner_name, v.phone_number, " +
    ...
    private JPanel buildEntryPanel() {
        ...
        fPhone.addKeyListener(new KeyAdapter() {
            @Override public void keyReleased(KeyEvent e) {
                ...
                if (ph.isEmpty()) { phoneHint.setText(" "); return; }
                if (!ph.matches("\\d{10}")) {
        ...
        statusLbl.setFont(F_BODY); statusLbl.setForeground(C_ACCENT);
        ...
        g.gridx = 1; g.gridy = 4; g.gridwidth = 1; form.add(phoneHint, g);
        g.gridx = 0; g.gridy = 5; g.gridwidth = 2; form.add(btnSlots, g);
        addRow(form, g, 6, "Select Slot *",    cSlot);
        g.gridy = 7; form.add(btnPark,   g);
        g.gridy = 8; form.add(statusLbl, g);
        g.gridy = 9; form.add(btnBack,   g);
        ...
        btnPark.addActionListener(e -> {
            ...
            if (!ph.isEmpty() && !ph.matches("\\d{10}")) {
                ...
                statusLbl.setForeground(C_DANGER); return;
            }
            ...
            if (res.startsWith("OK")) {
                fVehNo.setText(""); fOwner.setText(""); fPhone.setText(""); phoneHint.setText(" ");
            }
        });
    ...
    private void fillSlots(JComboBox<String> cb, String type) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(
            ...
            int count = 0;
            while (r.next()) { cb.addItem(r.getString("slot_number") + " (Rs" + r.getDouble("hourly_rate") + "/hr)"); count++; }
            if (count == 0) cb.addItem("No slots available for " + type);
        } catch (SQLException e) { err(e.getMessage()); }
    }

    private String doEntry(String vno, String vtype, String owner, String phone, String slot) {
        if (vno.isEmpty()) return "Vehicle number required";
        if (slot == null || slot.startsWith("No slots")) return "Select a valid slot";
        try (PreparedStatement chk = conn.prepareStatement(
                "SELECT 1 FROM vehicles WHERE vehicle_number=? AND status='Parked'")) {
            chk.setString(1, vno);
            if (chk.executeQuery().next()) return "Vehicle " + vno + " is already parked!";
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, vno); ps.setString(2, vtype);
    ...
    private JPanel buildExitPanel() {
        ...
        details.setFont(F_MONO); details.setEditable(false);
        details.setBackground(new Color(13, 17, 23)); details.setForeground(C_TEXT);
        ...
        result.setFont(F_BODY); result.setAlignmentX(CENTER_ALIGNMENT);

        payForm.add(sectionLabel("Payment"));
        payForm.add(Box.createVerticalStrut(16));
        payForm.add(lHours); payForm.add(Box.createVerticalStrut(6));
        payForm.add(lRate);  payForm.add(Box.createVerticalStrut(16));
        payForm.add(lAmount); payForm.add(Box.createVerticalStrut(14));
        payForm.add(fieldLabel("Manual Override (Rs) - for discounts"));
        payForm.add(Box.createVerticalStrut(4)); payForm.add(fManual);
        payForm.add(Box.createVerticalStrut(14));
        payForm.add(fieldLabel("Payment Method"));
        payForm.add(Box.createVerticalStrut(6)); payForm.add(cMethod);
        payForm.add(Box.createVerticalStrut(10)); payForm.add(btnCalc);
        payForm.add(Box.createVerticalStrut(8));  payForm.add(btnProcess);
        payForm.add(Box.createVerticalStrut(6));  payForm.add(result);
        payForm.add(Box.createVerticalStrut(6));  payForm.add(btnBack);
        right.add(payForm);
        ...
        btnSearch.addActionListener(e -> {
            ...
            if (vno.isEmpty()) { details.setText("Enter a vehicle number."); return; }
            try (PreparedStatement ps = conn.prepareStatement(
                ...
                ResultSet r = ps.executeQuery();
                if (r.next()) {
                    vid[0] = r.getInt("id"); rate[0] = r.getDouble("hourly_rate");
        ...
        btnCalc.addActionListener(e -> {
            if (vid[0] == -1) { result.setText("Search a vehicle first."); return; }
            try (PreparedStatement ps = conn.prepareStatement("SELECT entry_time FROM vehicles WHERE id=?")) {
                ps.setInt(1, vid[0]);
                ResultSet r = ps.executeQuery();
                if (r.next()) calcAmount(lHours, lRate, lAmount, r.getTimestamp("entry_time"), rate[0]);
            } catch (SQLException ex) { err(ex.getMessage()); }
        });
        ...
        btnProcess.addActionListener(e -> {
            ...
            try {
                ...
                if (doExit(vid[0], amt, (String) cMethod.getSelectedItem())) {
                    ...
                    details.setText(""); fSearch.setText(""); fManual.setText(""); vid[0] = -1;
                    lAmount.setText("Rs 0.00"); lHours.setText("Parking Duration: --");
                    lRate.setText("Hourly Rate: --");
                }
            } catch (NumberFormatException ex) {
                result.setText("Invalid amount"); result.setForeground(C_DANGER);
            }
        });
    ...
    private boolean doExit(int vid, double amount, String method) {
        try {
            ...
            try (PreparedStatement p = conn.prepareStatement(
                ...
                p.setDouble(1, amount); p.setInt(2, vid); p.executeUpdate();
            }
            try (PreparedStatement p = conn.prepareStatement(
                    "UPDATE parking_slots SET status='Available' WHERE id=" +
                    "(SELECT slot_id FROM vehicles WHERE id=?)")) {
                p.setInt(1, vid); p.executeUpdate();
            }
            try (PreparedStatement p = conn.prepareStatement(
                    "INSERT INTO payments(vehicle_id,amount,payment_method) VALUES(?,?,?)")) {
                p.setInt(1, vid); p.setDouble(2, amount); p.setString(3, method); p.executeUpdate();
            }
    ...
    private JPanel buildSlotsPanel() {
        ...
        JTextField fSlotNo = darkField("e.g. A7"); fSlotNo.setPreferredSize(new Dimension(100, 34));
        JComboBox<String> cType = darkCombo("Car","Motorcycle","Truck","Bus","Auto");
        JTextField fRate = darkField("Rate Rs/hr"); fRate.setPreferredSize(new Dimension(100, 34));
        JButton btnAdd     = smallButton("Add Slot",  C_ACCENT);
        JButton btnRefresh = smallButton("Refresh",   C_ACCENT2);
        JButton btnBack    = smallButton("Back",      C_SURFACE);
        JLabel  addStatus  = new JLabel(" "); addStatus.setForeground(C_ACCENT);

        addForm.add(fieldLabel("Slot:")); addForm.add(fSlotNo);
        addForm.add(fieldLabel("Type:"));  addForm.add(cType);
        addForm.add(fieldLabel("Rs/hr:")); addForm.add(fRate);
        addForm.add(btnAdd); addForm.add(btnRefresh); addForm.add(btnBack); addForm.add(addStatus);
        ...
        tbl.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(
                ...
                if (!sel && "Available".equals(st)) setForeground(C_ACCENT);
                else if (!sel && "Occupied".equals(st)) setForeground(C_WARN);
                setBorder(BorderFactory.createEmptyBorder(4, 8, 4, 8)); setFont(F_BODY);
                return this;
            }
        });

        JScrollPane scroll = new JScrollPane(tbl);
        scroll.setBorder(null); scroll.getViewport().setBackground(C_CARD);
        JPanel tableCard = roundedPanel(C_CARD, 12);
        tableCard.setLayout(new BorderLayout());
        ...
        btnAdd.addActionListener(e -> {
            ...
            if (sn.isEmpty() || rt.isEmpty()) {
                addStatus.setText("Fill all fields"); addStatus.setForeground(C_DANGER); return;
            }
            try (PreparedStatement ps = conn.prepareStatement(
                    "INSERT INTO parking_slots(slot_number,slot_type,hourly_rate) VALUES(?,?,?)")) {
                ps.setString(1, sn); ps.setString(2, tp); ps.setDouble(3, Double.parseDouble(rt));
                ps.executeUpdate();
                addStatus.setText("Slot " + sn + " added"); addStatus.setForeground(C_ACCENT);
                fSlotNo.setText(""); fRate.setText(""); loadSlots();
            } catch (Exception ex) { addStatus.setText(ex.getMessage()); addStatus.setForeground(C_DANGER); }
        });
    ...
    private JPanel buildReportsPanel() {
        ...
        JButton btnBack      = smallButton("Back",       C_SURFACE);
        controls.add(fieldLabel("Period:")); controls.add(cPeriod);
        controls.add(fieldLabel("Method:")); controls.add(cMethod);
        controls.add(btnGen); controls.add(btnExportCSV); controls.add(btnExportTXT); controls.add(btnBack);
        ...
        btnGen.addActionListener(e ->
            loadReportFiltered((String)cPeriod.getSelectedItem(),(String)cMethod.getSelectedItem(),lVeh,lRev,lAvg));
    ...
    private void loadReportFiltered(String period, String method,
        ...
        String sql =
            "SELECT p.payment_time, v.vehicle_number, v.vehicle_type, ps.slot_number, " +
        ...
        try (Statement s = conn.createStatement(); ResultSet r = s.executeQuery(sql)) {
            SimpleDateFormat sdf = new SimpleDateFormat("dd-MM HH:mm");
            int cnt = 0; double total = 0; long totalMins = 0;
            while (r.next()) {
                cnt++; double amt = r.getDouble("amount"); total += amt;
                long mins = r.getLong("mins"); totalMins += mins;
    ...
    private void exportReport(String format) {
        if (mdl_report.getRowCount() == 0) {
            JOptionPane.showMessageDialog(this, "Generate a report first!", "Nothing to export", JOptionPane.WARNING_MESSAGE);
            return;
        }
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(fc.getSelectedFile()))) {
            ...
            for (int c = 0; c < mdl_report.getColumnCount(); c++) headers.add(mdl_report.getColumnName(c));
            pw.println(String.join(sep, headers));
            if ("txt".equals(format)) pw.println("-".repeat(80));
    ...
    private JPanel buildHistoryPanel() {
        ...
        JButton btnBack   = smallButton("Back",       C_SURFACE);
        bar.add(fSearch); bar.add(btnSearch); bar.add(btnAll); bar.add(btnExport); bar.add(btnBack);

        String[] cols = {"ID","Vehicle No","Type","Owner","Phone","Slot","Entry","Exit","Duration","Amount","Status"};
        ...
        tbl.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(
                ...
                if (!sel && "Parked".equals(st)) setForeground(C_ACCENT);
                if (!sel && "Exited".equals(st)) setForeground(C_TEXT_DIM);
                setBorder(BorderFactory.createEmptyBorder(4, 8, 4, 8)); setFont(F_BODY);
                return this;
            }
        });
    ...
    private JPanel buildOverduePanel() {
        ...
        JPanel bar = roundedPanel(C_CARD, 12);
        bar.setLayout(new FlowLayout(FlowLayout.LEFT, 12, 12));
        JTextField fHours = darkField("6"); fHours.setPreferredSize(new Dimension(60, 34));
        JButton btnLoad   = smallButton("Find Overdue", C_DANGER);
        JButton btnExport = smallButton("Export CSV",   C_WARN);
        JButton btnBack   = smallButton("Back",         C_SURFACE);
        JLabel  hint      = new JLabel("Vehicles parked longer than:");
        hint.setFont(F_BODY); hint.setForeground(C_TEXT_DIM);
        bar.add(hint); bar.add(fHours); bar.add(fieldLabel("hours"));
        bar.add(btnLoad); bar.add(btnExport); bar.add(btnBack);

        String[] cols = {"ID","Vehicle No","Type","Owner","Phone","Slot","Entry Time","Hours Parked","Est. Charge"};
        ...
        JScrollPane scroll = new JScrollPane(tbl);
        scroll.setBorder(null); scroll.getViewport().setBackground(new Color(60, 20, 20));
        ...
        warn.setFont(F_BODY); warn.setForeground(C_WARN);
        ...
        btnLoad.addActionListener(e -> {
            try { loadOverdueFor(Integer.parseInt(fHours.getText().trim())); }
            catch (NumberFormatException ex) { err("Enter a valid number of hours"); }
        });
    ...
    private JPanel buildSettingsPanel() {
        ...
        JPasswordField fOld  = new JPasswordField(); styleField(fOld);
        JPasswordField fNew  = new JPasswordField(); styleField(fNew);
        JPasswordField fCnf  = new JPasswordField(); styleField(fCnf);
        JButton btnSave = accentButton("Save Password", C_ACCENT);
        JButton btnBack = smallButton("Back", C_SURFACE);
        JLabel  status  = new JLabel(" ");
        status.setFont(F_BODY); status.setAlignmentX(CENTER_ALIGNMENT);

        card.add(sectionLabel("Change Admin Password"));
        card.add(Box.createVerticalStrut(20));
        card.add(fieldLabel("Username"));         card.add(Box.createVerticalStrut(6)); card.add(fUser);
        card.add(Box.createVerticalStrut(14));
        card.add(fieldLabel("Current Password")); card.add(Box.createVerticalStrut(6)); card.add(fOld);
        card.add(Box.createVerticalStrut(14));
        card.add(fieldLabel("New Password"));     card.add(Box.createVerticalStrut(6)); card.add(fNew);
        card.add(Box.createVerticalStrut(14));
        card.add(fieldLabel("Confirm New"));      card.add(Box.createVerticalStrut(6)); card.add(fCnf);
        card.add(Box.createVerticalStrut(24));
        card.add(btnSave); card.add(Box.createVerticalStrut(8));
        card.add(status);  card.add(Box.createVerticalStrut(8)); card.add(btnBack);
        ...
        btnSave.addActionListener(e -> {
            ...
            if (!newP.equals(cnf)) { status.setText("Passwords dont match"); status.setForeground(C_DANGER); return; }
            if (newP.length() < 6) { status.setText("Min 6 characters");     status.setForeground(C_DANGER); return; }
            if (!authAdmin(u, oldP)) { status.setText("Wrong current password"); status.setForeground(C_DANGER); return; }
            try (PreparedStatement ps = conn.prepareStatement(
                    "UPDATE admin SET password=? WHERE username=?")) {
                ps.setString(1, newP); ps.setString(2, u); ps.executeUpdate();
                status.setText("Password updated"); status.setForeground(C_ACCENT);
                fOld.setText(""); fNew.setText(""); fCnf.setText("");
            } catch (SQLException ex) { status.setText(ex.getMessage()); status.setForeground(C_DANGER); }
        });
        btnBack.addActionListener(e -> showCard("Dashboard"));
        center.add(card);
        root.add(center, BorderLayout.CENTER);
    ...
    private JPanel buildTopBar(String pageTitle) {
        ...
        logo.setFont(new Font("Dialog", Font.BOLD, 18)); logo.setForeground(C_ACCENT);

        JLabel page = new JLabel(pageTitle);
        page.setFont(F_LABEL); page.setForeground(C_TEXT_DIM); page.setHorizontalAlignment(JLabel.CENTER);

        JPanel right = new JPanel(new FlowLayout(FlowLayout.RIGHT, 8, 0));
        right.setBackground(C_SURFACE);
        JLabel lClock = new JLabel();
        lClock.setFont(F_MONO); lClock.setForeground(C_TEXT_DIM);
        ...
        btnLogout.addActionListener(e   -> { currentUser = ""; showCard("Login"); });

        right.add(lClock); right.add(Box.createHorizontalStrut(8));
        right.add(btnDash); right.add(btnSettings); right.add(btnLogout);
    ...
    private void exportModel(DefaultTableModel model, String name) {
        if (model.getRowCount() == 0) {
            JOptionPane.showMessageDialog(this, "No data to export!", "Empty", JOptionPane.WARNING_MESSAGE); return;
        }
        JFileChooser fc = new JFileChooser();
        ...
        try (PrintWriter pw = new PrintWriter(new FileWriter(fc.getSelectedFile()))) {
            ...
            for (int c = 0; c < model.getColumnCount(); c++) h.add(model.getColumnName(c));
    ...
    //  UI HELPERS
    // ══════════════════════════════════════════════════════════════════════════════
    private JPanel darkPanel() { JPanel p = new JPanel(); p.setBackground(C_BG); return p; }

    private JPanel roundedPanel(Color bg, int arc) {
        return new JPanel() {
            { setOpaque(false); setBackground(bg); }
            @Override protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
                g2.setColor(bg); g2.fillRoundRect(0, 0, getWidth(), getHeight(), arc, arc); g2.dispose();
            }
        };
    }
    ...
    private JPanel statCard(String title, JLabel valueLabel, String icon, Color accent) {
        ...
        JLabel ico = new JLabel(icon + " " + title); ico.setFont(F_BODY); ico.setForeground(C_TEXT_DIM);
        valueLabel.setFont(new Font("Dialog", Font.BOLD, 26)); valueLabel.setForeground(accent);
        JPanel bar = new JPanel() {
            @Override protected void paintComponent(Graphics g) {
                Graphics2D g2 = (Graphics2D) g.create();
                g2.setColor(accent); g2.fillRoundRect(0, 0, getWidth(), getHeight(), 4, 4); g2.dispose();
            }
        };
        bar.setPreferredSize(new Dimension(0, 3)); bar.setOpaque(false);
    ...
    private JButton accentButton(String text, Color color) {
        ...
        b.setFont(F_LABEL); b.setForeground(Color.WHITE); b.setContentAreaFilled(false);
    ...
    private JButton smallButton(String text, Color bg) {
        ...
        b.setFont(F_BODY); b.setForeground(C_TEXT); b.setContentAreaFilled(false);
        b.setBorderPainted(false); b.setFocusPainted(false);
    ...
    private JButton navButton(String text) {
        ...
        b.setFont(new Font("Dialog", Font.BOLD, 13)); b.setForeground(C_TEXT);
    ...
    private JTextField darkField(String placeholder) { JTextField f = new JTextField(placeholder); styleField(f); return f; }

    private void styleField(JTextField f) {
        f.setBackground(C_BG); f.setForeground(C_TEXT); f.setCaretColor(C_ACCENT); f.setFont(F_BODY);
    ...
    private <T> JComboBox<T> darkCombo(T... items) {
        ...
        cb.setBackground(C_BG); cb.setForeground(C_TEXT); cb.setFont(F_BODY);
        ...
        cb.setRenderer((list, val, idx, sel, focus) -> {
            ...
            l.setOpaque(true); l.setFont(F_BODY);
            l.setBackground(sel ? C_ACCENT.darker() : C_BG); l.setForeground(C_TEXT);
            l.setBorder(BorderFactory.createEmptyBorder(4, 10, 4, 10)); return l;
        });
    ...
    private JLabel fieldLabel(String text) {
        JLabel l = new JLabel(text); l.setFont(F_LABEL); l.setForeground(C_TEXT_DIM);
        l.setAlignmentX(Component.LEFT_ALIGNMENT); return l;
    }

    private JLabel sectionLabel(String text) {
        JLabel l = new JLabel(text); l.setFont(F_TITLE); l.setForeground(C_TEXT);
        l.setAlignmentX(Component.LEFT_ALIGNMENT); return l;
    }

    private JLabel infoLabel(String text) {
        JLabel l = new JLabel(text); l.setFont(F_BODY); l.setForeground(C_TEXT_DIM);
        l.setAlignmentX(Component.LEFT_ALIGNMENT); return l;
    }
    ...
    private JTable styledTable(DefaultTableModel model) {
        ...
        t.setRowHeight(34); t.setGridColor(C_BORDER); t.setShowGrid(true);
        t.setSelectionBackground(C_ACCENT2.darker()); t.setSelectionForeground(C_TEXT);
        t.setFillsViewportHeight(true);
        JTableHeader h = t.getTableHeader();
        h.setBackground(C_SURFACE); h.setForeground(C_TEXT_DIM); h.setFont(F_LABEL);
        ...
        t.setDefaultRenderer(Object.class, new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(
                ...
                setBackground(sel ? C_ACCENT2.darker() : (row%2==0 ? C_CARD : new Color(28,36,46)));
                setForeground(C_TEXT); setFont(F_BODY);
    ...
    private void addRow(JPanel form, GridBagConstraints g, int row, String label, JComponent comp) {
        ...
        JLabel l = new JLabel(label); l.setFont(F_LABEL); l.setForeground(C_TEXT_DIM); form.add(l, g);
        g.gridx = 1; form.add(comp, g);
    }

    private String fmt(double v) { return String.format("%.2f", v); }
    private void err(String msg) { statusBar.setText("  " + msg); statusBar.setForeground(C_DANGER); }
